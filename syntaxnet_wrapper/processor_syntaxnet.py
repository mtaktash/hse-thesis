...
class ProcessorSyntaxNet(object):
    ...
    @staticmethod
    def _fill_spans_in_trees(sentences, trees):
    ...
    @staticmethod
    def _prepare_raw_input_for_syntaxnet(text, sentences):
        raw_input_s = ''
        if not sentences:
            raw_input_s = text + '\n\n'
        else:
    ...
    @staticmethod
    def _read_all_from_socket(sock):
        buf = bytes()
    ...
    @staticmethod
    def _parse_conll_format(string):
        try:
            result = list()
